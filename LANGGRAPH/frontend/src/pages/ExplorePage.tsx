...
export default function ExplorePage() {
  ...
  const { data: categoryData } = useCompanyCategories();

  const categoryOptions = categoryData?.map((item) => item.category).filter(Boolean) ?? [
    'Tech Giants',
    'Product Companies',
    'Service Companies',
    'Startups / Scale-ups',
  ...
  const { data, isLoading } = useCompanies(filters, sort);
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {companies.map((company) => (
            <CompanyCard key={company.id ?? company.short_name ?? company.name} company={company} />
          ))}
        </div>
      ) : (
        <EmptyState
